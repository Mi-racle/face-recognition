...
class ResidualSE(Module):
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1), se_reduct=4):
        ...
        for i in range(num_block):
            ...
            if i == num_block - 1:
                modules.append(
                    DepthWiseSE(c1_tuple, c2_tuple, c3_tuple, residual=True, kernel=kernel, padding=padding,
                                stride=stride,
                                groups=groups, se_reduct=se_reduct))
            else:
                modules.append(DepthWise(c1_tuple, c2_tuple, c3_tuple, residual=True, kernel=kernel, padding=padding,
...
class MiniFASNet(Module):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),
        ...
        self.conv_6_dw = LinearBlock(keep[47], keep[48], groups=keep[48], kernel=conv6_kernel, stride=(1, 1),
                                     padding=(0, 0))
